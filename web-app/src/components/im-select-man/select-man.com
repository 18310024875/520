		...
		<div class="gm-content">
			...
			<div class="gm-body">
				<div class="gm-dep-and-man-wrap">
					<ul>
						<li @click="this.changeActiveTab('team')"><span :class="{active : this.activeTab=='team'}">部门</span></li>
						<li @click="this.changeActiveTab('man')"><span :class="{active : this.activeTab=='man'}">成员</span></li>
					</ul>

					<!-- 部门组件 -->
					<teamContent v-show="this.activeTab=='team'"></teamContent>
	...
	*/
	...
	export default{
		...
		methods:{
			...
			submit(){
				let list = window.imSelectMan.selectedMans.map( v=>({
					avatar: v.avatar ,
					memberId : v.memberId,
					userName : v.userName,
				}))
				window.imSelectMan.callback ? window.imSelectMan.callback(list) : null ;
